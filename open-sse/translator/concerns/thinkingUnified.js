...
// An "openai" override describes effort semantics, not the wire shape, so Responses
// targets must still receive nested `reasoning: { effort }`. Conversely, do not put
// native Claude/Gemini/Kiro fields on an OpenAI-compatible wire.
function resolveFormat(targetFormat, model, provider) {
  const isResponsesTarget = targetFormat === "openai-responses" || targetFormat === "openai-response";
  const providerFmt = provider ? PROVIDERS[provider]?.thinkingFormat : null;
  const caps = getCapabilitiesForModel(provider, model);
  const configuredFmt = providerFmt || caps.thinkingFormat;
  const isOpenAIWire = targetFormat === "openai" || isResponsesTarget;

  if (configuredFmt === "openai" && isResponsesTarget) return "openai-responses";
  if (configuredFmt && !(isOpenAIWire && NATIVE_ONLY_FORMATS.has(configuredFmt))) {
